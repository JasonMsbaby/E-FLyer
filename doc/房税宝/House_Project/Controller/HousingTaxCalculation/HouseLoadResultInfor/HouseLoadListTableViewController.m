//
//  HouseLoadListTableViewController.m
//  House_Project
//
...
#import "LoadResultInforTableViewController.h"
@interface HouseLoadListTableViewController ()

@end
@implementation HouseLoadListTableViewController
static int flag = 0;
- (void)dealloc
{
    [_array release];
...
{
    NSMutableArray *arrayOne  = [DBManger selectBLInforAll];
    if (arrayOne.count == 0) {
        flag = 1;
...
{
    //获取要删除的对象
    BusinessLoanModel * blModel = [self.array objectAtIndex:indexPath.row];
    //删除数据库对应的记录
    [DBManger deleteBLInforWhereIndex:blModel];
    //将该对象从数组中移除
    [self.array removeObject:blModel];
    //移除cell
...
- (void)viewDidLoad
{
    ...
    self.array = [DBManger selectBLInforAll];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.dateLabel.text = [NSString stringWithFormat:@"商贷：%@",blModel.date];
    // Configure the cell...
...
{
    BusinessLoanModel * BLModel = [self.array objectAtIndex:self.array.count-indexPath.row-1];
    NSMutableArray *blModelArray = [NSMutableArray arrayWithObjects:BLModel.assessValue,BLModel.houseArea,BLModel.percentage,BLModel.year,BLModel.APR,BLModel.APRValue,BLModel.monthMoney,BLModel.loadMoney,BLModel.monthLoadMoney,nil];
    //跳详细结果页面

    LoadResultInforTableViewController *loadInforTVC = [[LoadResultInforTableViewController alloc]init];
    loadInforTVC.blarray = [NSMutableArray arrayWithArray:blModelArray];
