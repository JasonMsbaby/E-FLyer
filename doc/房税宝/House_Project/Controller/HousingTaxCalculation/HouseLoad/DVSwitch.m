...
{
    // to do
    return [[[DVSwitch alloc] initWithStringsArray:strings]autorelease];
}
...
- (instancetype)initWithStringsArray:(NSArray *)strings
{
    ...
    self.sliderView = [[[UIView alloc] init]autorelease];
    self.sliderView.backgroundColor = self.sliderColor;
    self.sliderView.clipsToBounds = YES;
    [self addSubview:self.sliderView];

    self.onTopLabels = [[[NSMutableArray alloc] init]autorelease];
...
- (void)sliderMoved:(UIPanGestureRecognizer *)rec
{
    if (rec.state == UIGestureRecognizerStateChanged) {
        ...
        NSMutableArray *distances = [[[NSMutableArray alloc] init]autorelease];
