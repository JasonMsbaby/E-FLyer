...
//    _colorTop = [[UIColor colorWithRed:255.0/255.0 green:255.0/255.0 blue:255.0/255.0 alpha:0.0]retain];
//    _colorLine = [[UIColor colorWithRed:0.0/255.0 green:191.0/255.0 blue:243.0/255.0 alpha:1]retain];
    _alphaTop = 1.0;
    _alphaBottom = 1.0;
    _alphaLine = 1.0;
    _widthLine = 1.0;
...
- (void)layoutSubviews {
    ...
    self.animationDelegate = [[[BEMAnimations alloc] init]autorelease];
    ...
    if (self.enableTouchReport == YES) {
        ...
        self.verticalLine = [[[UIView alloc] initWithFrame:CGRectMake(0, 0, 1, self.viewForBaselineLayout.frame.size.height)]autorelease];
...
- (BEMCircle *)closestDotFromVerticalLine:(UIView *)verticalLine {
    ...
    for (BEMCircle *dot in self.subviews) {
        ...
        if (dot.tag > 99 && dot.tag < 1000) {
            ...
            [UIView animateWithDuration:0.2 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
                dot.alpha = 0.5;
                dot.backRectView.backgroundColor = [UIColor whiteColor];
            } completion:nil];
            ...
            if (pow(((dot.center.x) - verticalLine.frame.origin.x), 2) < currentlyCloser) {
                ...
                dot.backRectView.backgroundColor = [UIColor yellowColor];
            }
        }
    }
...
- (void)drawXAxis {
    ...
    if (numberOfGaps >= (numberOfPoints - 1)) {
        UILabel *firstLabel = [[[UILabel alloc] initWithFrame:CGRectMake(3, self.frame.size.height - (labelXaxisOffset + 10), self.frame.size.width/2, 20)]autorelease];
        ...
        UILabel *lastLabel = [[[UILabel alloc] initWithFrame:CGRectMake(self.frame.size.width/2 - 3, self.frame.size.height - (labelXaxisOffset + 10), self.frame.size.width/2, 20)]autorelease];
        ...
        for (int i = 1; i <= (numberOfPoints/numberOfGaps); i++) {
            UILabel *labelXAxis = [[[UILabel alloc] init]autorelease];
